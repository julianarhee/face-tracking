...
def fit_glm(response_matrix, pred_matrix, pred_name_list, 
            ...
            opt = 'adam', device = '/gpu:0'):
    ''' Fit Poisson GLM
    Input:
    response_matrix: neural response data 
        (N timepoints x N cells)
    pred_matrix: design matrix 
        (N timepoints x N predictors)
    pred_name_list: list of predictor names 
        (optional)
    loss_type: loss type, 
        e.g. poisson (for calcium), gaussian (for averages), or binominal (for logistic regression)
    activation: activation function, e.g. exp, linear, or sigmoid
        pairs with loss_type: exponential (poisson), linear (gaussian), sigmoid (binomial)
    reg: regularization type, elastic_net or group_lasso
        default, use elastic_net
    l1_ratio: ratio for L1 regularization for elastic net
        1 is all L1, 0 is all L2 (default, use ~0.9)
    smooth_ratio: ratio for smooth penalty
        (skip)
    lambda_series: list of lambda values for regularization
        *start with largest regularization, end with smallest, or no-regularizatoin (order matters)
        larger lambda might underfit, lower lambda might overfit
    learning_rate: 
        tune this parameter if no convergence (if diffs bw iters jumps back and forth, lower rate)
        can tsart with ~0.01 
    convergence_tol: 
        increase to be less strict about convergence
    opt: e.g., 'adam', 'sdgm'
        adam faster
